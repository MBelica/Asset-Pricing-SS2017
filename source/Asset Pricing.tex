...
Ein Investor konsumiert in den zwei betrachteten Perioden $c_t, c_{t+1}$ und kann einen beliebigen Anteil $\xi$ der Zahlung $x_{t+1}$ kaufen oder verkaufen. Seien $e_t$, $e_{t+1}$ die Ausgangslevel an Konsum die der Investor ohne $x$ kaufen würden, z.B. festes Einkommenslevel vor Investitionen. Das Kalkül des Investors lautet dann:
	...
	unter den Nebenbedingungen: 
		...
		\end{align*}
Die Bedingung erster Ordnung ist damit:
...
Aus der first-order Bedingung folgt die zentrale Bewertungsgleichung
...
\begin{itemize}
	\item Bei einer sicheren Zahlung $x_{t+1}$, sei $R^f$ der risikofreie Zinssatz und so ergibt sich der Zusammenhang: $$p_t = \frac{1}{R^f} x_{t+1} = \frac{1}{1+r^f} x_{t+1}$$
...
Beachte allerdings, dass in first-order Bedingung nur ein Diskontfaktor vorkam, unabhängig vom möglichen Risiko oder betrachteten Asset, d.h. der stochastischer Diskontfaktor $m_{t+1}$
\begin{itemize}
