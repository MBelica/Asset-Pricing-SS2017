...
\begin{itemize}
	...
	\item $m = \frac{\beta u'(c_{t+1})}{u'(c_t)}$ eingesetzt liefert
		$$ \mathbb{E}\left[R^i \right] - R^f = - R^f \frac{\beta \cov(u'(c_{t+1}),R^i)}{u'(c_t)}  = -\frac{\cov(u'(c_{t+1}), R^i)}{\mathbb{E}[u'(c_{t+1})]} $$
\end{itemize}
